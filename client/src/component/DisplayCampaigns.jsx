...
const DisplayCampaigns = ({ title, isLoading, campaigns }) => {
    ...
    return (
        <div>
            <h1 className={`font-epilogue font-semibold text-[18px] ${theme?'text-black':'text-white'} text-left`}>{title} ({campaigns.length})</h1>
            <div className='grid sm:grid-cols-2 lg:flex lg:flex-wrap mt-[20px] gap-[26px]  '>
                ...
                {!isLoading && campaigns.length === 0 && (
                    <p className={`font-epilogue font-semibold text-[14px]leading-[30px] ${theme?'text-black':'text-[#818183]'}  capitalize`}>
