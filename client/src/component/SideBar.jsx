...
const SideBar = () => {
    ...
    const Icon = ({style,name,imageUrl,isActive,disabled,handleClick}) => (
        <div className={`w-[48px] h-[48px] rounded-[10px] ${isActive && isActive === name && 'bg-[#2c2f32]'} flex justify-center items-center ${!disabled && 'cursor-pointer'} ${style}`} onClick={handleClick}>
            {!isActive ? (
  ...
  return (
    <div className='flex justify-between items-center flex-col sticky top-5 h-[93vh]'>
        ...
        <div className='flex-1 flex flex-col justify-between items-center bg-[#1c1c24] rounded-[20px] w-[70px] py-4 mt-12'>
            ...
            <Icon imageUrl={sun} style={'bg-[#1c1c24] shadow-secondary'} />
        </div>
